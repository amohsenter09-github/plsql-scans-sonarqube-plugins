CREATE OR REPLACE PROCEDURE VMSCMS.SP_AUTHORIZE_TXN_CSR_AUTH_ACH (
...
/*************************************************************************************************
     ...
     * Modified Reason  : Logging of below details in statementlog table
                          1) ledger balance ,Acct Type,Timestamp  
     * Reviewer         : Dhiraj
     ...
     * Modified for     : DFCCSD 68 : Load Fee Not Being Shown
     * Modified Reason  : logging details in transactiionlog table in update statement.
    ...
    * Modified Date     : 30-Oct-14    
    * Modified For      : FSS-1831 and Review Changes
    * Reviewer          : Spankaj
    * Build Number      : 
...
--  v_appliocationprocess_stat   VARCHAR2 (3);
  ...
  V_CAM_TYPE_CODE   cms_acct_mast.cam_type_code%type; -- Added on 17-Apr-2013 for defect ?
 ...
 BEGIN
  ...
  BEGIN
     ...
     /*   BEGIN
        ...
        BEGIN
         ...
         SELECT CAM_ACCT_BAL, CAM_LEDGER_BAL, CAM_ACCT_NO,
                cam_type_code                                   -- Added on 17-Apr-2013 for defect ?
           INTO V_ACCT_BALANCE, V_LEDGER_BAL, V_CARD_ACCT_NO,
                v_cam_type_code                                 -- Added on 17-Apr-2013 for defect ?
           FROM CMS_ACCT_MAST
          WHERE CAM_ACCT_NO =
               (SELECT CAP_ACCT_NO
                 FROM CMS_APPL_PAN
   ...
   -------------------------------------------------------------
      ...
      /* -- not required since tran desc already fetched above
       ...
       v_timestamp := systimestamp;              -- Added on 17-Apr-2013 for defect ?
             ...
             BEGIN
               INSERT INTO CMS_STATEMENTS_LOG
                (CSL_PAN_NO,
                 ...
                 CSL_ACCT_TYPE,         -- Added on 17-Apr-2013 for defect ?
                 CSL_TIME_STAMP,         -- Added on 17-Apr-2013 for defect ?
                ...
                (
                 V_HASH_PAN,
                 V_LEDGER_BAL,      -- V_ACCT_BALANCE removed to use V_LEDGER_BAL on 17-Apr-2013 for defect ?
                 V_TRAN_AMT,
                 V_DR_CR_FLAG,
                 V_TRAN_DATE,
                 DECODE(V_DR_CR_FLAG,
                       'DR',
                       V_LEDGER_BAL - V_TRAN_AMT,   -- V_ACCT_BALANCE removed to use V_LEDGER_BAL on 17-Apr-2013 for defect ?
                       'CR',
                       V_LEDGER_BAL + V_TRAN_AMT,   -- V_ACCT_BALANCE removed to use V_LEDGER_BAL on 17-Apr-2013 for defect ?
                       'NA',    
                       V_LEDGER_BAL),               -- V_ACCT_BALANCE removed to use V_LEDGER_BAL on 17-Apr-2013 for defect ?
                 ...
                 v_cam_type_code,   -- Added on 17-Apr-2013 for defect ?
                 v_timestamp,        -- Added on 17-Apr-2013 for defect ?
  ...
  /*  BEGIN
    ...
    EXCEPTION
    WHEN EXP_REJECT_RECORD  --added by amit on 18-sep-2012
    THEN
        RAISE;
     WHEN OTHERS THEN
       V_GL_UPD_FLAG := 'N';
 ...
 commented on 18092012 Dhiraj Gaikwad As P_PROCESSTYPE value is default N
      ...
      BEGIN
        ...
        VALUES
         (p_MSG,
          ...
          p_BANK_CODE,
          TRIM(TO_CHAR(V_TOTAL_AMT, '99999999999999990.99')),
          NULL,
          NULL,
          p_MCC_CODE,
          p_CURR_CODE,
