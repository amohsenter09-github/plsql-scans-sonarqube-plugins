create or replace
PROCEDURE       VMSCMS.SP_SPIL_CARD_ACTIVATION (P_INSTCODE IN NUMBER,
                                          P_RRN      		 IN VARCHAR2,
...
BEGIN
  ...
  SELECT DECODE(UPPER(VOD_POSTBACK_RESPONSE),'TRUE','1','1','1','0')
    ...
    ||'~'
    ||Vod_Channel_Id,To_Number(Nvl(Vol_Denomination,'0')),VOL_PRODUCT_FUNDING,VOL_FUND_AMOUNT
  INTO p_POSTBACK_URL_OUT,v_lineitem_denom,v_order_prod_fund,v_order_fund_amt
...
END; 
  IF v_b2b_cardFlag='N' THEN 
      ...
      ELSE
        ...
        END IF;
      END IF;
  ELSIF V_CAP_CARD_STAT = '1' AND v_b2b_cardFlag = 'Y' THEN
      IF V_FIRSTTIME_TOPUP = 'Y' THEN
        V_RESPCODE := '9'; -- response for invalid transaction
        P_ERRMSG   := 'Card Activation Already Done For this Card';
        RAISE EXP_MAIN_REJECT_RECORD;
...
END IF  ;  
  ...
  BEGIN
    ...
    SELECT CPC_RELOADABLE_FLAG,CPC_PROD_DENOM, CPC_PDENOM_MIN, CPC_PDENOM_MAX,
        CPC_PDENOM_FIX,CPC_PROFILE_CODE, cpc_b2bcard_stat,  decode(nvl(cpc_b2b_flag,'N'),'N', cpc_retail_activation,0),CPC_PRODUCT_FUNDING,CPC_FUND_AMOUNT,cpc_user_identify_type
...
END IF;
 ...
 IF v_b2b_cardFlag = 'Y' THEN
   IF  (V_ACTIVATION_CODE IS NOT NULL AND P_ACTIVATION_CODE IS NULL ) OR 
       (V_ACTIVATION_CODE IS NOT NULL AND P_ACTIVATION_CODE IS NOT NULL AND V_ACTIVATION_CODE <> P_ACTIVATION_CODE ) THEN
          v_respcode := '292';
