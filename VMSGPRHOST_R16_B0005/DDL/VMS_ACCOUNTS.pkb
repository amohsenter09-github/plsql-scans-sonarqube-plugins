...
--Sn pan  and customer details
    ...
    BEGIN
     ...
     END IF;
      ...
      BEGIN
        sp_concurrent_preauthcheck(p_deposit_id_in||'~'||p_partner_id_in, p_trandate_in, l_hash_pan, p_inst_code_in, p_delivery_channel_in, l_concurrent_flag );
        IF (l_concurrent_flag > 0) THEN
          p_resp_code_out   := '191';
          l_errmsg          := 'Concurrent Transaction in Process';
          RAISE exp_reject_record;
        ELSE
          sp_autonomous_preauth_log(l_auth_id,p_deposit_id_in||'~'||p_partner_id_in, p_trandate_in, l_hash_pan, p_inst_code_in, p_delivery_channel_in );
        END IF;
      EXCEPTION
      WHEN exp_reject_record THEN
        RAISE;
