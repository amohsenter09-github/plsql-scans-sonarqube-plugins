   ...
   begin
    ...
    END;
     ...
     EXCEPTION
      ...
      BEGIN
        sp_concurrent_preauthcheck(P_DEPOSIT_ID||'~'||p_partner_id, P_TRAN_DATE, v_hash_pan, p_inst_code, p_delivery_channel, V_concurrent_flag );
        IF (V_concurrent_flag > 0) THEN
          v_resp_cde         := '191';
          v_err_msg          := 'Concurrent Transaction in Process';
          RAISE exp_reject_record;
        ELSE
          sp_autonomous_preauth_log(V_AUTH_ID,P_DEPOSIT_ID||'~'||p_partner_id, P_TRAN_DATE, v_hash_pan, p_inst_code, p_delivery_channel );
        END IF;
      EXCEPTION
      WHEN exp_reject_record THEN
        RAISE;
