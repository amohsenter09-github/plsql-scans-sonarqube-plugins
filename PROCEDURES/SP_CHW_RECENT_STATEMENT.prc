            ...
            THEN 
              ( SELECT DECODE(nvl(regexp_instr(sl.csl_trans_narrration,'RVSL-',1,1,0,'i'),0),0,A.TRANS_DESC,'RVSL-'||A.TRANS_DESC)                          
               ...
               || '/'
                                                                                                             ...
                                                                                                             ||'/'
                                                                                                           || A.auth_id)
                  FROM TRANSACTIONLOG A where customer_card_no = sl.CSL_PAN_NO and rrn = sl.csl_rrn and A.auth_id = sl.csl_auth_id
