...
PROCEDURE                                     VMSCMS.SP_TOPUP_PAN_CMS_SPIL (
                                       ...
                                       --Added for serial number changes on 29/10/14
                                        ...
                                        P_ACCT_BAL         OUT VARCHAR2,                                        
                                         ...
                                         p_merchant_zip    in varchar2 default null) AS-- added for VMS-622 (redemption_delay zip code validation)
  ...
  BEGIN
    ...
    IF V_RRN_COUNT >= 1 THEN
     ...
     WHEN EXP_MAIN_REJECT_RECORD THEN
       ...
       WHEN OTHERS THEN
        ...
        P_ERRMSG   := 'Error While Fetching Business date Details for RRN:' || P_RRN;
     ...
     EXCEPTION
       WHEN NO_DATA_FOUND THEN
        V_RESPCODE := '22';
        P_ERRMSG   := 'Error While Fetchin the RRN Details-No data found:' ||
                    P_RRN;
        RAISE EXP_DUPLICATE_REQUEST;
       WHEN TOO_MANY_ROWS THEN
        V_RESPCODE := '22';
        P_ERRMSG   := 'Error While Fetchin the RRN Details-More than one row return:' ||
                    P_RRN;
        RAISE EXP_DUPLICATE_REQUEST;
    ...
    WHEN EXP_DUPLICATE_REQUEST THEN
        RAISE;
            WHEN OTHERS THEN
             V_RESPCODE := '22';
             P_ERRMSG   := 'Error in RRN duplicate Verification:' ||
                        P_RRN;
             RAISE EXP_DUPLICATE_REQUEST;
          END;
        END LOOP;

        IF P_ERRMSG = 'OK' AND V_DUPCHK_COUNT = 0 THEN
          V_DUPCHK_COUNT := 1;
        END IF;
       EXCEPTION
     WHEN EXP_MAIN_REJECT_RECORD THEN
    ...
    WHEN EXP_DUPLICATE_REQUEST THEN
        ...
        WHEN OTHERS THEN
          V_RESPCODE := '22';
          P_ERRMSG   := 'Error in RRN in CURSOR selection:' || P_RRN;
          RAISE EXP_DUPLICATE_REQUEST;
       END;
     END IF;
    END IF;
