           ...
           THEN
              ...
              ELSE
                ...
                --SN VMS to approve all preauth transactions with MCCode - 5542 transactions as partially approved    FSS - 1278
                   IF P_MCC_CODE = '5542'
                   AND UPPER (TRIM(p_networkid_switch)) <>
                                     'PULSE'
                                            --Added on 17.09.2013 for FSS-1313
                   THEN
            -- code added to partial amount in response since 5542 preauth is considered as partial preauth
            --Added for MVHOST-926 on 19-May-2014
                 --   IF (( UPPER (TRIM(p_networkid_switch)) <> 'VISANET') OR P_ONEDOLLAR_PREAUTH IS NOT NULL) -- Modified for the Mantis ID:14808
                 --   THEN      --Commented for MCC-5542 Changes
                        p_partialauth_amount := v_tran_amt;
                        v_partial_appr := 'Y';
                --   END IF; --Commented for MCC-5542 Changes
                END IF;
                 --EN VMS to approve all preauth transactions with MCCode - 5542 transactions as partially approved    FSS - 1278
