create or replace
PROCEDURE        VMSCMS.SP_CSR_ORDER_REPLACE (
...
BEGIN
   ...
   BEGIN
      ...
      End;
       ...
       Begin
       Select Cpc_Renew_Replace_Option,cap_card_stat into v_renew_option,V_APPLPAN_CARDSTAT
         From Cms_Prod_Cattype,Cms_Appl_Pan Where Cpc_Inst_Code=Cap_Inst_Code And 
               Cpc_Prod_Code=Cap_Prod_Code And Cpc_Card_Type=Cap_Card_Type And Cap_Pan_Code=v_hash_pan ;
        
        
      If V_Renew_Option='SP' and V_APPLPAN_CARDSTAT <> '2' Then
      ...
      EXCEPTION
         ...
         THEN
            V_Resp_Msg :=
               'Error while finding Renew replace option ' || SUBSTR (SQLERRM, 1, 100);
            v_resp_code := '21';
            Raise Excp_Rej_Record;
      END;
      
      BEGIN
         sp_chw_order_replace (
                               prm_inst_code,
                               prm_msg,
                               prm_rrn,
      ...
      THEN
         --v_new_pan := TRIM (prm_resp_msg);
      IF prm_replacement_option != 'SP' THEN
         BEGIN
     ...
     IF prm_replacement_option != 'SP' THEN
         BEGIN
            ...
            v_new_value := v_new_detl_old_card || '~' || v_new_card_value;
         EXCEPTION
            WHEN OTHERS
            THEN
               v_resp_msg :=
         ...
         BEGIN
            ...
            INSERT INTO cms_calllog_details
                 ...
                 VALUES (prm_inst_code, prm_call_id, v_hash_pan,
                         ...
                         v_txn_code, prm_tran_date, prm_tran_time,
         ...
         BEGIN
            INSERT INTO cms_transaction_log_dtl
               ...
               --  VALUES (prm_delivery_channel, prm_txn_code, NULL,
                  VALUES (prm_delivery_channel, v_txn_code, NULL,
