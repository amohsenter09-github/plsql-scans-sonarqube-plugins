  ...
  BEGIN
   ...
   V_MS_PYMNT_TYPE:=null;
    ...
    --SN:Added for 4.0.4 performance changes
      BEGIN
        sp_concurrent_preauthcheck(p_stan, p_tran_date, v_hash_pan, p_inst_code, p_delivery_channel, V_concurrent_flag );
        IF (V_concurrent_flag > 0) THEN
          v_resp_cde         := '191';
          v_err_msg          := 'Concurrent Transaction in Process';
          RAISE exp_reject_record;
        ELSE
          sp_autonomous_preauth_log(V_AUTH_ID,p_stan, p_tran_date, v_hash_pan, p_inst_code, p_delivery_channel );
        END IF;
      EXCEPTION
      WHEN exp_reject_record THEN
        RAISE;
