create or replace
PROCEDURE               VMSCMS.SP_AUTHORIZE_TXN_SPIL_BALINQ(
  ...
  /*************************************************
     ...
     * modified Defect  : Defect 10406
     * modified reason  : To ignore max card balance check
     * Reviewer         : Dhiarj
     * Reviewed Date    : 27-FEB-13
     * Build Number     : CMS3.5.1_RI0023.2_B0009
     ...
     * Modified reason  : NVl added for Fee amount logging
     * Reviewer         : Dhiraj
 ...
 /* CURSOR C_MINI_TRAN IS
  ...
  --V_MINRRNDATE      varchar2(20);--TRANSACTIONLOG.BUSINESS_DATE%TYPE;--Commeted duplicate rrn check
  --V_MAXRRNDATE      varchar2(20);--TRANSACTIONLOG.BUSINESS_DATE%TYPE;--Commeted duplicate rrn check
...
BEGIN
  ...
  V_RESP_CDE := '1';
  P_ERR_MSG  := 'Success'; -- Response Message to have Success message if Response Code is 00 for SPIL
  P_RESP_MSG := 'OK';
  ...
  IF P_SERIAL_NUMBER IS NOT NULL THEN
    BEGIN
      ...
      EXCEPTION
       ...
       WHEN OTHERS THEN
         V_RESP_CDE := '21';
         V_ERR_MSG  := 'Error while validating serial number '||substr(sqlerrm,1,200);
         RAISE EXP_REJECT_RECORD;
    END;
  END IF;
...
--Modified to retrieve from cms_prod_cattype instead of cms_bin_level_config since international transactions will be tied to product category henceforth
    ...
    IF V_DR_CR_FLAG <> 'NA' THEN
     ...
     BEGIN
       ...
       -- V_ERR_MSG  := 'Error while calling SP_DAILY_BIN_BAL ' ||
         ...
         V_ERR_MSG  := 'Error while calling SP_DAILY_BIN_BAL ' ||V_HASH_PAN;
        RAISE EXP_REJECT_RECORD;
       END IF;
    ...
    IF V_TOTAL_FEE <> 0 OR V_FREETXN_EXCEED = 'N' THEN -- Modified by Trivikram on 26-July-2012 for logging fee of free transaction
     ...
     EXCEPTION
       WHEN OTHERS THEN
        ...
        --V_ERR_MSG  := 'Error while selecting data from card Master for card number ' ||
          --          P_CARD_NO; commented for FSS-2320.
          V_ERR_MSG  := 'Error while selecting data from card Master for card number ' || V_HASH_PAN;
 ...
 /*   IF V_OUTPUT_TYPE = 'B' THEN
     ...
     IF P_TXN_CODE = '04' AND P_DELIVERY_CHANNEL = '07' THEN
       ...
       EXCEPTION
        WHEN OTHERS THEN
          V_ERR_MSG  := 'Problem while fetching data from C_MINI_TRAN cursor ' ||
                     SUBSTR(SQLERRM, 1, 300);
          V_RESP_CDE := '21';
          RAISE EXP_REJECT_RECORD;
       END;
    ...
    BEGIN
     ...
     EXCEPTION
       WHEN NO_DATA_FOUND THEN
        ...
        RAISE EXP_REJECT_RECORD;
       WHEN OTHERS THEN
        V_ERR_MSG  := 'Error while seleting CMS_TRANSLIMIT_CHECK' ||
                    SUBSTR(SQLERRM, 1, 200);
        V_RESP_CDE := '21';
        RAISE EXP_REJECT_RECORD;
     END;
...
EXCEPTION
    ...
    /* BEGIN
     ...
     EXCEPTION
       WHEN OTHERS THEN
        P_ERR_MSG  := 'Problem while selecting data from response master ' ||
                    V_RESP_CDE || SUBSTR(SQLERRM, 1, 300);
        P_RESP_CODE := '69';
        ...
        IF TO_NUMBER(P_DELIVERY_CHANNEL) = 8 THEN
          BEGIN
            SELECT RESPONSE_CODE
             INTO V_DUPRRN_RESP_CODE --V_RESP_CDE  --modified by Pankaj S. on 26_Feb_2013
     ...
     EXCEPTION
       WHEN OTHERS THEN
        P_ERR_MSG  := 'Problem while inserting data into transaction log  dtl' ||
                    SUBSTR(SQLERRM, 1, 300);
  ...
  BEGIN
    ...
    VALUES
     (P_MSG,
      ...
      V_PROD_CODE,
      V_PROD_CATTYPE,
      TRIM(TO_CHAR(nvl(P_TIP_AMT,0), '99999999999999990.99')), -- trim(to_chart added for defect 10871
      P_DECLINE_RULEID,
      P_ATMNAME_LOC,
      V_AUTH_ID,
      V_TRANS_DESC,
      ...
      P_INST_CODE,
      V_FEE_CODE,
      nvl(V_FEE_AMT,0),            -- NVL Added on 10-Dec-2013 for 13160
      ...
      P_RVSL_CODE,
      V_ACCT_NUMBER,
      ROUND(nvl(V_UPD_AMT,0),2),         --Added for defect 10871   --Modified by Revathi on 02-APR-2014 for 3decimal place issue
      ROUND(nvl(V_UPD_LEDGER_AMT,0),2), --Added for defect 10871   --Modified by Revathi on 02-APR-2014 for 3decimal place issue
      P_INTERNATIONAL_IND,
      V_RESP_CDE,
  ...
  EXCEPTION
    WHEN OTHERS THEN
     ...
     P_ERR_MSG  := 'Problem while inserting data into transaction log  ' ||
                 SUBSTR(SQLERRM, 1, 300);
  END;
    ...
    IF P_SERIAL_NUMBER IS NOT NULL THEN
      BEGIN
       ...
       EXCEPTION
         WHEN OTHERS THEN
          P_RESP_CODE := '69';
          P_ERR_MSG  := 'Error while calling SP_SPIL_SERIALNUMBER_LOGGING ' || SUBSTR(SQLERRM, 1, 300);
          ROLLBACK;
      END;
    END IF;
...
EXCEPTION
  WHEN OTHERS THEN
    ROLLBACK;
    P_RESP_CODE := '69'; -- Server Declined
    P_ERR_MSG  := 'Main exception from  authorization ' ||
                SUBSTR(SQLERRM, 1, 300);
END;
