CREATE OR REPLACE PROCEDURE VMSCMS.SP_TRAN_FEES_PRODUCT_CMSAUTH
...
prm_mcc_code         IN          VARCHAR2, -- Added by Trivkram on 08 Sept 2012
...
/***************************************************************************************
    ...
    * Modified Reason  :  Mantis ID:12219 : v_tran_code and v_tran_type variable used but assigned value from 
                         cfm_tran_code the default fees calculated based on per transaction not all the transaction 
                         in delivery channel.
    * Reviewer         :  DHIRAJ
    ...
    * Modified for     : Mantis id:12277
    * Modified Reason  : Transaction Fee should be assessed based on sysdate 
    * Reviewer         : Dhiraj
...
v_count number;
  BEGIN
   prm_freetxn_exceeded := 'Y'; --Added by Trivikram on 27-July-2012 for default value
     BEGIN
    ...
    if v_card_activation='Y' then
        ...
        BEGIN -- BEGIN 1
            SELECT  cfm_fee_code,  cfm_fee_amt, cfm_per_fees, cfm_min_fees,
                     ...
                     CFM_CLAWBACK_FLAG,cpf_fee_plan,CFM_NORMAL_RVSL,cfm_tran_mode,CFM_MERC_CODE,CFM_MAX_LIMIT,CFM_MAXLMT_FREQ,
                   cfm_tran_code,cfm_tran_type,cfm_cap_amt, --Added cfm_tran_code,cfm_tran_type for mantis id:12213 on 05-09-2013 & added cfm_cap_amt for lyfehost-61
                ...
                INTO    prm_fee_code,  prm_flat_fee, prm_per_fees, prm_min_fees,
                        ...
                        prm_fee_plan,v_reversal_code,v_tran_mode,v_merc_code,v_maxlimit,v_maxlimitfreq,
                        v_tran_code,v_tran_type,v_cap_amt, --Added v_tran_code,v_tran_type for mantis id:12213 on 05-09-2013
                        PRM_FEE_DESC
                        ,v_frrefreq_change --Added for OTC changes
            FROM CMS_FEE_MAST, CMS_PROD_FEES, cms_fee_types, cms_fee_feeplan
           ...
           --Commented and modified  on 06.09.2013 for 12277
           AND ((cpf_valid_to IS NOT NULL AND (trunc(sysdate) between cpf_valid_from and cpf_valid_to))
             ...
             OR (cpf_valid_to IS NULL AND trunc(sysdate) >= cpf_valid_from))     --Commented and modified  on 06.09.2013 for 12277
            ...
            OR prm_del_channel NOT IN ('02','05')) or prm_reversal_flag='R')--Modified by Deepa on July 04 2012 as the CFM_NORMAL_RVSL will be NULL for the Non-Financial txns             
            ...
            AND (((prm_del_channel='02') AND (prm_reversal_flag='N') AND 
             ...
             WHEN NO_DATA_FOUND THEN
              ...
              RAISE exp_nofees; -- NO FEES ATTACHED RETURN -1
                --SN Added on 06.09.2013 
            WHEN OTHERS THEN
                    prm_error := 'ERROR FROM MAIN 0 =>' || SQLERRM    ;
                    RAISE exp_main;         
            --EN Added on 06.09.2013 
            END;
         WHEN  exp_main THEN--Added on 05.09.2013 
           RAISE;    
            WHEN OTHERS THEN
      ...
      --Commented by Deepa on Sep-15-2012 as the Domestic/International txn based fee is added in the query itself
       /* IF v_intl_indicator='A' OR  prm_intl_indicator=v_intl_indicator THEN
        ...
        --Commented by Deepa on Sep-15-2012 as the PIN/Signature based fee is added in the query itself
...
--                 END IF;
                ...
                --SN Commented and modified  on 06.09.2013 for 12277
                 ...
                 --EN Commented and modified  on 06.09.2013 for 12277    
      ...
      ----------------------------------------------------------------------------------------      
--START FOR  NCGPR-438 by Naila on 14-08-2013
