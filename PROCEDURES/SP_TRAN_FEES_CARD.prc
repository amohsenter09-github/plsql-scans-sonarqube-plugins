create or replace
PROCEDURE              vmscms.SP_TRAN_FEES_CARD
...
prm_mcc_code            IN          VARCHAR2, -- Added by Trivikram on 08 Sept 2012
...
/***************************************************************************************************
      ...
      * Modified Reason  :  Mantis ID:12219 : v_tran_code and v_tran_type variable used but assigned value from  cfm_tran_code 
                           the default fees calculated based on per transaction not all the transaction in delivery channel.
      * Reviewer         :  Dhiraj
      ...
      * Modified Reason  : Merchandise return & Preauth completion 
      * Reviewer         : Spankaj
      ...
      * Modified for     : 
      * Modified Reason  : Calculate fee for ATM auth & compl transactions
      * Reviewer         : Pankaj
      * Reviewed Date    :
...
exp_nofees          EXCEPTION        ;
 ...
 v_free_txncnt      CMS_FEE_MAST.CFM_FREE_TXNCNT%TYPE default 0;
...
v_card_cnt          number; --Added on 02.09.2013 for DFCHOST-340
...
--EN CREATE HASH PAN
 v_tran_date:= TRUNC(prm_tran_date);
  ...
  EXCEPTION
  WHEN OTHERS THEN
        prm_error := 'Error while selecting count from card level fee --'||SUBSTR(SQLERRM,1,200) ;
        RAISE exp_main;    
  END;
   --EN Added on 02.09.2013 for DFCHOST-340
   IF v_card_cnt= 0 THEN --IF condition Added on 02.09.2013 for DFCHOST-340
...
BEGIN
    ...
    BEGIN -- BEGIN 1
        ...
        SELECT cfm_fee_code, cfm_fee_amt, cfm_per_fees, cfm_min_fees, cce_crgl_catg,
               cce_crgl_code, cce_crsubgl_code, cce_cracct_no, cce_drgl_catg,
               cce_drgl_code, cce_drsubgl_code, cce_dracct_no, cce_st_calc_flag,
               cce_cess_calc_flag, cce_st_cracct_no, cce_st_dracct_no,
               cce_cess_cracct_no, cce_cess_dracct_no, cfm_feeamnt_type,
               cfm_duration, cfm_free_txncnt,CFM_INTL_INDICATOR,CFM_APPROVE_STATUS,
               CFM_PIN_SIGN,CFM_CLAWBACK_FLAG,cce_fee_plan,CFM_NORMAL_RVSL,cfm_tran_mode,CFM_MERC_CODE,CFM_MAX_LIMIT,CFM_MAXLMT_FREQ, --modified for ncgpr-438 by Naila on 14-08-2013
               cfm_tran_code,cfm_tran_type,cfm_cap_amt, --Added cfm_tran_code,cfm_tran_type for mantis id:12213 on 05.09.013 null cfm_cap_amt for lyfehost-61
               cfm_fee_desc -- Added for MVCSD-4471
               ,cfm_duration_change --Added for OTC changes
          INTO prm_fee_code, prm_flat_fee, prm_per_fees, prm_min_fees, prm_crgl_catg,
               prm_crgl_code, prm_crsubgl_code, prm_cracct_no, prm_drgl_catg,
               prm_drgl_code, prm_drsubgl_code, prm_dracct_no, prm_st_calc_flag,
               prm_cess_calc_flag, prm_st_cracct_no, prm_st_dracct_no,
               prm_cess_cracct_no, prm_cess_dracct_no, prm_feeamnt_type,
               v_duration, v_free_txncnt,v_intl_indicator,v_approve_stat,
               v_PIN_TXN,prm_clawback,prm_fee_plan,v_reversal_code,v_tran_mode,v_merc_code,v_maxlimit,v_maxlimitfreq ,--modified for ncgpr-438 by Naila on 14-08-2013
               v_tran_code,v_tran_type,v_cap_amt, --Added v_tran_code,cfm_tran_type for mantis id:12213 on 05.09.013
         ...
         WHERE cfm_inst_code = prm_inst_code
          ...
          -- AND prm_tran_date >= cce_valid_from
           --AND ((CCE_VALID_TO IS NOT NULL AND (v_tran_date between cce_valid_from and CCE_VALID_TO))
            ...
            --OR (CCE_VALID_TO IS NULL AND v_tran_date >= cce_valid_from))  --Modified by Deepa on Aug 16 2012 to have two active FeePlan
            OR (CCE_VALID_TO IS NULL AND trunc(sysdate)  >= cce_valid_from)) --Commented and modified  on 05.09.2013 for 12277
    ...
    EXCEPTION -- EXCEPTION 1
         ...
         --Added by Deepa on Sep-25-2012 To calculate the MCC code Fee for the configuration of MCCode(ALL)         
               ...
               cfm_duration, cfm_free_txncnt,CFM_INTL_INDICATOR,CFM_APPROVE_STATUS,
               CFM_PIN_SIGN,CFM_CLAWBACK_FLAG,cce_fee_plan,CFM_NORMAL_RVSL,cfm_tran_mode,CFM_MERC_CODE,CFM_MAX_LIMIT,CFM_MAXLMT_FREQ, --modified for ncgpr-438 by Naila on 14-08-2013
               cfm_tran_code,cfm_tran_type,cfm_cap_amt, --Added cfm_tran_code,cfm_tran_type for mantis id:12213 on 05.09.013  cfm_cap_amt for lyfehost-61
               cfm_fee_desc -- Added for MVCSD-4471
               ,cfm_duration_change --Added for OTC changes
          INTO prm_fee_code, prm_flat_fee, prm_per_fees, prm_min_fees, prm_crgl_catg,
               prm_crgl_code, prm_crsubgl_code, prm_cracct_no, prm_drgl_catg,
    ...
    WHEN  exp_main THEN--Added on 05.09.2013 for DFCHOST-340(review)
       RAISE;
    WHEN OTHERS THEN
            prm_error := 'ERROR FROM MAIN 1 =>' || SQLERRM    ;
            RAISE exp_main;
    ...
    --Commented by Deepa on Sep-15-2012 as the PIN/Signature txnbased fee is added in the query itself
...
--                 END IF;
                             ...
                             WHERE  add_ins_date between to_date(v_from_date||'000000','yyyymmddhh24miss') and to_date(v_to_date||'235959','yyyymmddhh24miss')
                              ...
                              AND ((prm_reversal_flag='N' AND ((v_tran_mode='A' and txn_mode in (0,1))
                               ...
                               AND ((prm_del_channel IN ('01','02') AND ((v_intl_indicator='A' AND INTERNATION_IND_RESPONSE IN('0','1'))
                                ...
                                AND ((prm_del_channel IN ('02') AND ((v_PIN_TXN='A' AND pos_verification IN('P','S'))
                                OR (pos_verification=v_PIN_TXN))) or delivery_channel not IN ('02'))
                               -- AND (((prm_del_channel IN ('02','05') AND (MCCODE=prm_mcc_code OR  MCCODE='A')) 
                               AND (((prm_del_channel IN ('02','05') AND (MCCODE=prm_mcc_code OR  v_merc_code='A'))--Modified by Deepa on Sep-20 2012
                               /*MCCODE in transactionlog will not have the value 'A'.Fee Configured value in cms_fee_mast should be checked with 'A'(ALL)*/
                                /*OR prm_del_channel NOT IN ('02','05')) or prm_reversal_flag='R');--Modified by Deepa on Sep-17-2012 to change the reversal flag value from 'Y' to 'R'
                                --Added by Deepa on July 03 2012 for checking International Indicator and POS verification
        ...
        Raise exp_main; --Added on 05.09.2013 for DFCHOST-340(review)
        END;
      END IF;
...
--START FOR  NCGPR-438
      IF v_tran_fee=1 AND v_maxlimit>0 THEN
          ...
          SP_ACCTLVL_FEELIMIT(prm_inst_code,
            ...
            END IF;
       END IF;
  --END FOR NCGPR-438 by Naila on 14-08-2013
      IF v_tran_fee=1 THEN
