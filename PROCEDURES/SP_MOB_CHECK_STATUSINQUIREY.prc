create or replace PROCEDURE        vmscms.sp_mob_check_statusinquirey (
...
AS
   ...
   v_check_stat_res    CLOB;
   v_check_stat_val    CLOB;
   ...
   CURSOR c_check_status (
      ...
      p_custmer_id   VARCHAR2,
      p_from_date    VARCHAR2,
      p_to_date      VARCHAR2
   )
   IS
      SELECT      TO_CHAR (TO_DATE (DECODE (cct_txn_flag,
                                            '0', cct_pend_date,
                                            cct_act_date
                                           ),
                                    'YYYY/MM/DD'
               ...
               || '~'
               || TO_CHAR (TO_DATE (DECODE (cct_txn_flag,
                                            '0', cct_pend_time,
                                            cct_act_time
                                           ),
                                    'HH24MISS'
               ...
               || cct_check_desc
               || '~'
               || TRIM (TO_CHAR (DECODE (cct_txn_flag,
                                         '0', cct_pend_amt,
                                         cct_txn_amnt
                                        ),
                                 999999990.99
         ...
         WHERE cct_cust_id = p_custmer_id
           AND cct_inst_code = p_inst_code
           AND TRUNC (cct_ins_date) BETWEEN TO_DATE (p_from_date, 'MMDDYYYY')
                                        AND TO_DATE (p_to_date, 'MMDDYYYY')
      ORDER BY cct_ins_date DESC;
      /**********************************************************************************************
...
BEGIN
   ...
   BEGIN
      ...
      --En generate auth id
      BEGIN
         OPEN c_check_status (p_inst_code, p_cust_id, p_from_date, p_to_date);

         LOOP
            FETCH c_check_status
             INTO v_check_stat_val;

            EXIT WHEN c_check_status%NOTFOUND;
            v_check_stat_res :=
                               v_check_stat_res || ' || ' || v_check_stat_val;
         END LOOP;
      ...
      EXCEPTION
         ...
         THEN
            v_err_msg :=
                  'Problem while selecting data from c_check_status cursor'
               || SUBSTR (SQLERRM, 1, 300);
            v_resp_cde := '21';
            RAISE exp_reject_record;
      END;

      IF (v_check_stat_res IS NULL)
      THEN
         v_check_stat_res := ' ';
      ELSE
         v_check_stat_res :=
                      SUBSTR (v_check_stat_res, 5, LENGTH (v_check_stat_res));
      END IF;
