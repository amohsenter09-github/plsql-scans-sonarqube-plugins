...
BEGIN
  ...
  SELECT DECODE(UPPER(ordr.VOD_POSTBACK_RESPONSE),'TRUE','1','1','1','0')
    ...
    ||'~'
    ||ordr.Vod_Channel_Id,To_Number(Nvl(lineitem.Vol_Denomination,'0')),lineitem.VOL_PRODUCT_FUNDING,lineitem.VOL_FUND_AMOUNT
...
--Sn added duplicate rrn check.
    ...
    exception
        ...
        BEGIN
            SELECT CAM_ACCT_BAL,NVL(cam_defund_flag,'N')
            INTO V_ACCT_BALANCE,v_defund_flag
            FROM CMS_ACCT_MAST
