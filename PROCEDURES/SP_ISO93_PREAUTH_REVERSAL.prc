...
/*  BEGIN-- Sn Commented on 01/07/2013 BY Arunprasath 
  ...
  Begin
  sp_concurrent_preauthcheck(p_org_stan, P_ORGNL_BUSINESS_DATE,
   V_HASH_PAN,  P_INST_CODE, P_DELV_CHNL,V_concurrent_flag );
   if (V_concurrent_flag > 0) then 
        V_RESP_CDE     := '191';
        V_ERRMSG       := 'Concurrent Transaction in Process';
        RAISE EXP_RVSL_REJECT_RECORD;
  Else
       sp_autonomous_preauth_log(V_AUTH_ID,p_org_stan, P_ORGNL_BUSINESS_DATE,
        V_HASH_PAN,  P_INST_CODE, P_DELV_CHNL );
  End if;
  Exception
  When EXP_RVSL_REJECT_RECORD Then 
  raise;
