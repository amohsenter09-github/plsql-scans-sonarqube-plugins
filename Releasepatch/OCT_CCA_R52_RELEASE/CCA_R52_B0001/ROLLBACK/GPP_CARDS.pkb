...
********************************************************************************************/
    ...
    l_card_id               vmscms.cms_prod_cattype.cpc_card_id%TYPE;
  ...
  BEGIN
    ...
    --  'N')
      INTO l_token_eligibility,
           ...
           l_product_id,
           l_profile_code,
           l_card_id --, l_cvvplus_eligibility
      FROM vmscms.cms_prod_cattype
     WHERE cpc_inst_code = 1
       AND cpc_prod_code = l_prod_code
       AND cpc_card_type = l_card_type;

    IF l_user_type IN ( '1', '4') and NVL(l_form_factor,'P') <> 'V'
    THEN
      CASE
...
********************************************************************************************/
    ...
    --Performance Fix
        ...
        IF l_flag = 1 THEN
            p_status_out := vmscms.gpp_const.c_mandatory_status;
            g_err_mandatory.RAISE(l_api_name,',0002,',l_field_name || ' is mandatory');
            p_err_msg_out := g_err_mandatory.get_current_error;
                vmscms.gpp_cards.log_transactionlog (l_api_name,
                                                   p_customer_id_in,
                                                   l_hash_pan,
