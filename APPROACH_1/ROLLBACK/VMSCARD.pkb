   ...
   BEGIN
      p_resp_msg_out := 'OK';
          ...
          SELECT cap_pan_code, fn_dmaps_main (cap_pan_code_encr)
           ...
           WHERE cap_prod_code = p_prod_code_in              --VMS-1084 (Pan genaration process from sequential to shuffled - B2B & Retail)
...
if p_minval_in =0 then
  ...
  --commented for VMS-1084(Pan genaration process from sequential to shuffled - B2B & Retail)
 /*         BEGIN
...
--    l_serlno       vms_pan_ctrl_inv.vpc_ctrl_numb%TYPE;
   BEGIN
      ...
      BEGIN
         SELECT vpc_ctrl_numb, vpc_max_serlno
           INTO p_serial_no_out, l_max_serlno
           FROM vms_pan_ctrl_inv
          WHERE     vpc_inst_code = p_inst_code_in
                AND vpc_prod_code = p_prod_code_in
                AND vpc_prod_catg = p_prod_catg_in
...
/*      IF p_serl_flag = 1
      ...
      IF l_max_serlno > p_serial_no_out
      THEN
